    ...
    <table class="table table-bordered table-sm" id="clientes-maestra-general-admin">
        <thead>
            <tr class="text-center" style="font-size: 14px; background-color: #c22121; color: white;">
                <th><input type="checkbox" onchange="ActualizarValoresCheck()" id="checkSelectTodos"> <i class="fas fa-mouse-pointer"></i></th>
                ...
                <tr>
                    <td>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="declinarCliente[]" id="declinar{{ $value->id_cliente }}"
                    ...
                    <td style="cursor: pointer">
                        <div class="btn-group" role="group" aria-label="Basic example">
                            <i class="fas fa-star start1{{ $value->tipo_cliente }} star{{ $value->id_cliente }}" id="cl{{ $value->id_cliente }}1"
                                onclick="validarEstella('1', '{{ $value->id_cliente }}')" title="Oportunidad"></i>
                            <i class="fas fa-star start2{{ $value->tipo_cliente }} star{{ $value->id_cliente }}" id="cl{{ $value->id_cliente }}2"
                                onclick="validarEstella('2', '{{ $value->id_cliente }}')" title="Prospecto"></i>
                            <i class="fas fa-star start3{{ $value->tipo_cliente }} star{{ $value->id_cliente }}" id="cl{{ $value->id_cliente }}3"
                        ...
                        @if ($value->tipo_cliente != '0')
                            <div class="btn-group" role="group" aria-label="Basic example">
                                <button type="button" class="btn btn-sm btn-info" title="Ver información cliente"
                                    onclick="ObtenerInformacionCliente('{{ $value->id_cliente }}')"><i class="fas fa-eye"
                                        style="font-size: 15px"></i></button>
                                <button type="button" class="btn btn-sm btn-primary" title="Programar una llamada"
                                    onclick="AgendarLlamadaCliente('{{ $value->id_cliente }}')"><i class="fas fa-phone-alt"
                                        style="font-size: 15px"></i></button>
                                <a href="{{ route('asesor.whatsapp', ['celular' => empty($value->celular_1) ? 1 : $value->celular_1]) }}"
                                    target="_BLANK" type="button" class="btn btn-sm btn-success" title="Enviar mensaje al WhatsApp"><i
                            ...
                            <div class="btn-group" role="group" aria-label="Basic example">
                                @if ($value->tipo_cliente == '3')
                                    <button type="button" class="btn btn-sm btn-warning" id="ventaEfectiva{{ $value->id_cliente }}"
                                        ...
                                        title="Marcar venta efectiva"><i class="far fa-money-bill-alt" style="font-size: 15px"></i></button>
                                @else
                                    <button type="button" class="btn btn-sm btn-warning" id="ventaEfectiva{{ $value->id_cliente }}"
                                ...
                                <a href="{{ route('send.encuesta.asesor', ['celular' => empty($value->celular_1) ? 1 : $value->celular_1, 'nombre' => empty($nombre_c) ? 'Albura' : $nombre_c]) }}"
                                    target="_BLANK" type="button" class="btn btn-sm btn-secondary" title="Enviar encuesta de satisfacción"><i
                                        class="fas fa-paper-plane" style="font-size: 15px"></i></a>
                                <button type="button" class="btn btn-sm btn-danger"
                                    onclick="NotificarEliminacionClienteCrmAdmin('{{ $value->id_cliente }}')" title="Eliminar contacto"><i
