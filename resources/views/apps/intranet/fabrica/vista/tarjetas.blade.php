            ...
            <div class="card-body" id="card-color">
                <div class="post">
                    <div class="user-block " id="user-block">
                        ...
                        <a href="{{ asset($item['url']) }}" target="_BLANK"><img id="border-image" class="border-image card-img-top img-fluid"
                                src="{{ asset($item['url']) }}" alt="Imagen Idea"></a>
                            ...
                            <div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-left" aria-labelledby="dropdownMenuButton">
                                @if ($item['self_usuario'] == $item['id_usuario'])
                                    <span class="dropdown-item d-flex " id="span-comment">
                                        <a type="button" title="Eliminar Idea"
                                            onclick="deleteIdea('{{ $item['id'] }}', '{{ route('delete.prototipo') }}')"> <i
                                                class="fas fa-trash-alt" style="color: #dc143c"> </i>
                                            <small> Eliminar Idea</small> </a></span>


                                    <span class="dropdown-item d-flex " id="span-comment">
                                        <a id="cambio_{{ $item['id'] }}" onclick="puente('{{ $item['id'] }}','{{ $item['nombre'] }}')"
                                            type="button" title="Editar Idea" data-toggle="modal" data-id-idea="{{ $item['id'] }}"
                                            data-target="#modal_cambios" data-link="{{ $item['link'] }}"><i class="fas fa-edit"
                                                style="color: rgb(245, 181, 64)"></i>
                                            <small> Editar Idea</small> </a></span>

                                    @if (isset($item['link']))
                                        <span class="dropdown-item d-flex " id="span-comment">
                                            <a onclick="deleteLink('{{ $item['id'] }}','{{ route('delete-link') }}')" type="button"
                                                title="Eliminar Imagen" data-toggle="modal" data-id-idea="{{ $item['id'] }}"><i
                                ...
                                <span class="dropdown-item  d-flex  ">
                                    <a id="id_ideas{{ $item['id'] }}" type="button" title="Comentarios de esta idea"
                                        onclick="validarInformacion('{{ $item['id'] }}','{{ route('comentarios.prototipos') }}')"
                    ...
                    @if (isset($item['link']))
                        <center class="mt-2">
                            <span id="enlace"><a href="{{ $item['link'] }}" target="_blank"><img src="{{ asset('img/www.png') }}" alt=""
                                        width="25" height="25">
