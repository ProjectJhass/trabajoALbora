...
<div class="card mb-4 mb-lg-0">
        ...
        <div class="card-title d-flex align-items-start justify-content-between">
            ...
            @if ($info_st->estado != 'definido')
                <div class="demo-inline-spacing">
                    <div class="btn-group">
                        <button type="button" class="btn btn-danger btn-icon rounded-pill dropdown-toggle hide-arrow" data-bs-toggle="dropdown"
                            aria-expanded="false">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            ...
                            <li><a class="dropdown-item" id="add{{ $id_ost }}" href="javascript:void(0);"
                                    onclick="modalAddComentariosAdicionales('{{ $id_ost }}')">Agregar comentario general</a></li>
                            <li><a class="dropdown-item" id="print{{ $id_ost }}" target="_BLANK"
                                    href="{{ route('print.form.solicitud', ['id_st' => $id_ost]) }}">Imprimir formato OST</a></li>
                            <li><a class="dropdown-item" id="update{{ $id_ost }}" {{ empty($comentarios_v) ? '' : 'hidden' }}
                                    href="javascript:void(0);" onclick="updateOrdenStVisitaEvidencias('{{ $id_ost }}')">Actualizar visita /
                                    Evidencias</a></li>
                            @if ($info_st->proceso == 'Servicio tecnico' || $info_st->proceso == 'Almacen' || $info_st->proceso == 'Cliente')
                                @if (in_array($info_st->estado, $estados_) || ($info_st->respuesta_st == 'No garantia' || $info_st->respuesta_st == 'No garantia por tiempo'))
                                    <li><a class="dropdown-item" id="printFab{{ $id_ost }}" target="_BLANK"
                                            href="{{ route('print.carta', ['id_st' => $id_ost]) }}">Imprimir respuesta de
                                            fábrica</a>
                                    </li>
                                    @if (Auth::user()->almacen == 'HAPPYSLEEP')
                                        <li><a class="dropdown-item" id="printFab{{ $id_ost }}"
                                                href="{{ route('format.hs', ['id_st' => $id_ost]) }}">Imprimir respuesta
                                                HS</a>
                                        </li>
                                    @endif
                                @endif
                                @if (
                                    ...
                                    <li><a class="dropdown-item" id="sendNotify{{ $id_ost }}" href="javascript:void(0);"
                                            onclick="sendNotificacionCarta('{{ $id_ost }}')">Enviar notificación y carta</a>
                                    </li>
                                @endif
                            @endif
                            @if ($info_st->proceso == 'Fabrica' && $info_st->estado == 'En valoracion')
                                <li><a class="dropdown-item" id="valoracion{{ $id_ost }}" href="javascript:void(0);"
                                        onclick="modalValoracionFabrica('{{ $id_ost }}')">Emitir concepto de valoración</a></li>
                            @endif
                            @if ($info_st->proceso == 'Fabrica' && $info_st->estado == 'Carta en elaboracion')
                            ...
                            @if ($info_st->proceso == 'Almacen' && $info_st->estado == 'Recoger')
                                <li><a class="dropdown-item" id="recoger{{ $id_ost }}"
                                        onclick="modalUpdateDataRecogida('{{ $id_ost }}')" href="javascript:void(0);">Actualizar recogida</a>
                                </li>
                            @endif
                            @if ($info_st->respuesta_st == 'No garantia' && $info_st->proceso == 'Servicio tecnico' && $info_st->estado == 'Por definir')
                                <li><a class="dropdown-item" id="updateValFab{{ $id_ost }}" href="javascript:void(0);"
                                        onclick="modalUpdateValoracionGerOst('{{ $id_ost }}')">Actualizar valoración
                                        fábrica</a></li>
                            @endif
    ...
    @foreach ($data as $item)
        <div class="card-body" style="font-size: 16px">
            <div class="row">
                ...
                <div class="col-md-4">
                    <p class="mb-1"><strong>Número ST: </strong><span class="badge rounded-pill bg-label-primary">{{ $item->id_st }}</span></p>
                ...
                <div class="col-md-4">
                    <p class="mb-1"><strong>Estado: </strong><span class="badge rounded-pill bg-label-info">{{ $item->estado }}</span></p>
                    <p class="mb-1"><strong>Nombre: </strong> {{ $item->nombre }}</p>
                    <p class="mb-1"><strong>Email: </strong> {{ $item->email }}</p>
                    <p class="mb-1"><strong>Dirección: </strong> {{ $item->direccion . ' - ' . $item->barrio . ' - ' . $item->ciudad }}</p>
                ...
                <div class="col-md-4">
                    <p class="mb-1"><strong>Concepto: </strong><span class="badge rounded-pill bg-label-info">{{ $item->respuesta_st }}</span></p>
        ...
        <div class="divider">
            ...
            <div class="demo-inline-spacing">
                <button class="btn rounded-pill btn-secondary"
                    onclick="consultarInformacionEvidencias('visita','{{ $id_ost }}','{{ $info_st->estado }}')" type="button"
                    data-bs-toggle="offcanvas" data-bs-target="#offcanvasStart" aria-controls="offcanvasStart">
            ...
            @foreach ($valoracion as $item)
                <li class="d-flex mb-4 pb-1">
                    ...
                    <div class="d-flex w-100 flex-wrap align-items-center justify-content-between gap-2">
                        <div class="me-2">
                            <h6 class="mb-0">{{ date('Y-m-d', strtotime($item->created_at)) . ' - ' . $item->observaciones }}</h6>
    ...
    <div class="card-header">
        <div class="divider">
            ...
            <div class="demo-inline-spacing">
                <button class="btn rounded-pill btn-secondary"
                    onclick="consultarInformacionEvidencias('recogida','{{ $id_ost }}','{{ $info_st->estado }}')" type="button"
                    data-bs-toggle="offcanvas" data-bs-target="#offcanvasStart" aria-controls="offcanvasStart">
                    <span class="tf-icons bx bx-camera me-1"></span>Evidencias
                </button>
            </div>
    ...
    <div class="card-header">
        <div class="divider">
            ...
            <div class="demo-inline-spacing">
                <button class="btn rounded-pill btn-secondary"
                    onclick="consultarInformacionEvidencias('taller_ingreso','{{ $id_ost }}','{{ $info_st->estado }}')" type="button"
                    data-bs-toggle="offcanvas" data-bs-target="#offcanvasStart" aria-controls="offcanvasStart">
                    <span class="tf-icons bx bx-camera me-1"></span>Evidencias ingreso
                </button>
                <button class="btn rounded-pill btn-danger"
                    onclick="consultarInformacionEvidencias('taller_reparado','{{ $id_ost }}','{{ $info_st->estado }}')" type="button"
                    data-bs-toggle="offcanvas" data-bs-target="#offcanvasStart" aria-controls="offcanvasStart">
    ...
    <div class="card-header divider">
        ...
        <div class="demo-inline-spacing">
            <button class="btn rounded-pill btn-secondary" type="button" data-bs-toggle="offcanvas" data-bs-target="#offDocumentoSoporteSt"
                aria-controls="offDocumentoSoporteSt">
...
<div class="offcanvas offcanvas-start" style="min-width: 600px" tabindex="-1" id="offcanvasStart" aria-labelledby="offcanvasStartLabel">
...
<div class="offcanvas offcanvas-start" style="min-width: 850px" tabindex="-1" id="offDocumentoSoporteSt" aria-labelledby="offcanvasStartLabel">
    <div class="offcanvas-header">
